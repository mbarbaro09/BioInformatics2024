...
```{r}
#for jaccard i tried to calculate it manually or by using the ml3measures library, in both cases it returns the same value
#we need to understand why and if the metric is actually necessary.
jaccard <- function(truth, prediction) {
  intersection <- length(intersect(truth, prediction))
  union <- length(truth) + length(prediction) - intersection
  intersection / union
}

#Fowlkes-Mallows index
FMI <- function(precision, recall) { sqrt(precision * recall) }
...
truth <- factor(labels, levels=sort(unique(labels)));
compute_metrics <- function(prediction) {
  metrics_row <- mclustcomp(pam_results[[i]]$clustering, labels, types=types);
  
  df <- tibble(actual = truth, predicted = prediction)
  precision <- yardstick::precision(df, truth=actual, estimate=predicted, estimator="macro")$.estimate
  recall <- yardstick::recall(df, truth=actual, estimate=predicted, estimator="macro")$.estimate
  
  #accuracy, bal_acc_macro, precision, recall, F1, rand_index, adj_rand_index, nmi, purity, mvi, v_measure, jaccard, FMI
  c(yardstick::accuracy(df, truth=actual, estimate=predicted, estimator="macro")$.estimate,
    yardstick::bal_accuracy(df, truth=actual, estimate=predicted, estimator="macro")$.estimate,
    precision,
    recall,
    yardstick::f_meas(df, truth=actual, estimate=predicted, estimator="macro", beta=1)$.estimate,
    FMI(precision, recall),
    metrics_row$scores[3], 
    metrics_row$scores[1], 
    metrics_row$scores[2], 
    funtimes::purity(truth, prediction)$pur, 
    mcclust::vi.dist(truth, prediction), 
    clevr::v_measure(truth, prediction),
    mlr3measures::jaccard(list(as.integer(truth), as.integer(prediction))))
}

#the micro_average metrics are all equals because of the symmetry of the confusion matrix and they are equal to the macro_accuracy
metrics <- matrix(ncol = 13, nrow = 0)
colnames(metrics) <- c("accuracy", "bal_accuracy_macro", "precision", "recall", "f1", "FMI", "rand", "adjrand", "nmi1", "purity", "mvi", "v_measure", "jaccard")
...
for (i in 1:length(pam_results)) {
  ...
  confusionMatrixes[[i]] <- yardstick::conf_mat(df, truth=actual, estimate=predicted);
}
...
#use min-max normalization to adjust MVI w.r.t. to the interval [0, log(n)]
#then invert it in order to indicate that the if two clustering solutions are more similar the value is higher
metrics[, "mvi"] <- 1 - (metrics[, "mvi"]) / (log(length(truth)))
...
clean_name <- function(text) {
  substrings_to_remove <- c(".scores1", ".scores2", ".scores3")

  # Create a regex pattern by pasting the substrings together
  pattern <- paste(substrings_to_remove, collapse = "|")
  
  # Remove the substrings from the string
  gsub(pattern, "", text)
}
df_metrics <- as.data.frame(metrics);

df <- data.frame(
  value = unlist(df_metrics[, seq(2, ncol(df_metrics), by = 2)]),
  class = sapply(names(unlist(df_metrics[, seq(2, ncol(df_metrics), by = 2)])), clean_name)
)


df

# Plot histogram with categories and classes
ggplot(df, aes(x = value, fill = class)) +
  geom_bar(position = "dodge") +  # Dodge separates the bars for each class
  labs(x = "Metric", y = "Cluster similarity", fill = "Technique") +
  theme_minimal()
```
