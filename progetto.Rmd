...
for(i in 1:length(W_list)) {
  ...
  clustering_names[[pam_next_count]] <- paste("PAM", sub("-.*", "", names(complete)[[i]]))
...
colnames(metrics) <- c("FMI", "rand", "adjrand", "nmi1", "jaccard", "purity", "mvi")
...
internal_metrics <- matrix(ncol = 1, nrow = 0)
colnames(internal_metrics) <- c("Average Silhouette Width")

for(i in 1:3) {
  internal_metrics <- rbind(internal_metrics, 
                            c(summary(silhouette(pam_results[[i]], as.dist(1-W_list[[i]])))$avg.width))
}

internal_metrics <- rbind(internal_metrics, 
                          c(summary(silhouette(pam_results[[4]], as.dist(1-W_averages)))$avg.width))
internal_metrics <- rbind(internal_metrics, 
                          c(summary(silhouette(pam_results[[5]], as.dist(1-W_int)))$avg.width))
internal_metrics <- rbind(internal_metrics, 
                          c(summary(silhouette(pam_results[[6]], as.dist(1-W_NEMO)))$avg.width))
internal_metrics <- rbind(internal_metrics, 
                          c(summary(silhouette(NEMO_spectral_clustering, as.dist(1-W_NEMO)))$avg.width))
internal_metrics <- rbind(internal_metrics, 
                          c(summary(silhouette(SNF_spectral_clustering, as.dist(1-W_int)))$avg.width))
...
```{r}
internal_metrics[,1] <- sapply(internal_metrics[,1], round, digits = 3) 
metrics[,] <- sapply(metrics[,], round, digits = 4) 

save_table <- function(data, columns, filename) {
  df_data <- as.data.frame(data)
  df_data <- cbind("Clustering Method" = rownames(data), df_data)

  gt_table <- gt(df_data) %>%
    tab_header(
      title = "Internal Cluster validation measures Table"
    ) %>%
    tab_style(
...
save_table(internal_metrics, 2, "internal_metrics_table")
save_table(metrics, 8, "external_metrics_table")
```