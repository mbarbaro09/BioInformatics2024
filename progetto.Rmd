...
for(i in 1:length(complete))
{
    ...
    W_list[[i]] <- SNFtool::affinityMatrix(distances_matrix);
}
...
W_int <- SNFtool::SNF(W_list, K=20, t=20);
```
...
compute_metrics <- function(prediction, df) {
  ...
  #accuracy, bal_acc_macro, precision, recall, F1, FMI, rand_index, adj_rand_index, nmi, purity, mvi, v_measure
...
metrics <- matrix(ncol = 12, nrow = 0)
colnames(metrics) <- c("accuracy", "bal_accuracy_macro", "precision", "recall", "f1", "FMI", "rand", "adjrand", "nmi1", "purity", "mvi", "v_measure")
...
for (i in 1:length(pam_results)) {
  ...
  metrics <- rbind(metrics, compute_metrics(prediction, df))
...
df <- tibble(actual = truth, predicted = prediction)
metrics <- rbind(metrics, compute_metrics(prediction, df))
confusionMatrixes[[counter]] <- yardstick::conf_mat(df, truth=actual, estimate=predicted);
